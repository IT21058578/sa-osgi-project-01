		...
		try {
			...
			while (true) {
				...
				case 1:
					...
					System.out.println("+-----+---------------------+");
					db.getServiceProviders().stream().forEach((provider) -> {
					...
					System.out.print("2->" + "Enter data package Name:");
					String name = reader.readLine();
					System.out.print("3->" + "Enter data package Description:");
					String description = reader.readLine();
					System.out.print("4->" + "Enter data package Price:");
					String pr = reader.readLine();

					// cast to double
					double price = Double.parseDouble(pr);

					DataPlan plan = new DataPlan(ProviderID, name, description, price);
				...
				case 2:
					...
					if (!data.isEmpty()) {
						...
						System.out
								.println("+-----+-----+---------------------+----------------------+----------------+");
						db.getDataPlans().stream().forEach((item) -> {
							System.out.printf("| %-2s | %-2s | %-19s | %-20s | %-11.11s |\n", item.getProviderId(),
				...
				case 4:
					...
					boolean packageDeleted = db.deleteDataPackage(id);
					if (packageDeleted) {
						System.out.println("==============================");
					...
					if (!searchResults.isEmpty()) {
						...
						for (IDataPlan result : searchResults) {
							System.out.printf("| %-2s | %-2s | %-19s | %-20s | %-11.2s |\n", result.getProviderId(),
									result.getId(), result.getName(), result.getDescription(), result.getPrice());
							System.out.println(
									"+-----+-----+---------------------+----------------------+----------------+");
